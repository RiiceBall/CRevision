...
void show_monsters(void)
{
	...
	// 打印怪物信息
	printf("当前地图出现了如下怪物：");
	set_cursor_position(MARGIN_X + 5, INFO_START_Y + 1);
	for (i = 0; i < curr_map_monster_count; i++)
	{
		...
		{
			// 移动光标至下个怪物打印的起始位置
			set_cursor_position(MARGIN_X + 5, INFO_START_Y + 1 + i / 3);
		}
		printf("%s(%s)\t", monster_array[monster_index[i]].name, level_names[monster_array[monster_index[i]].level - 1]);
	}
}
